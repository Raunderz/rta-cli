...
class MemorizeTool(BaseTool[MemorizeParams]):
    ...
    async def execute(self, params: MemorizeParams, cancel_event: asyncio.Event | None = None) -> ToolResult:
        data = _load()
        data[params.key] = {
            "value": params.value, 
        ...
        return ToolResult(
            success=True,
            result=f"Memorized: {params.key}",
            ui_summary=f"Memorized {params.key}"
        )
...
class RecallTool(BaseTool[RecallParams]):
    ...
    async def execute(self, params: RecallParams, cancel_event: asyncio.Event | None = None) -> ToolResult:
        data = _load()
        if not data:
        ...
        return ToolResult(
            success=True,
            result=result_text,
            ui_summary=f"Found {len(matches)} matches"
        )
...
class ForgetTool(BaseTool[ForgetParams]):
    ...
    async def execute(self, params: ForgetParams, cancel_event: asyncio.Event | None = None) -> ToolResult:
        data = _load()
        if params.key in data:
            ...
            return ToolResult(
                success=True, 
                result=f"Forgot: {params.key}",
                ui_summary=f"Forgot {params.key}"
            )
