...
def test_tree_navigation_branches_without_overwriting(tmp_path, monkeypatch):
    ...
    session.append_message(AssistantMessage(content=[TextContent(text="root answer")], stop_reason=StopReason.STOP))
    ...
    assert [m.content for m in session.messages if isinstance(m, UserMessage)] == ["root", "branch"]
