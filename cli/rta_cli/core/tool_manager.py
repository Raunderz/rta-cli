...
class ToolManager:
    ...
    async def execute_call(
        self, tool_call: ToolCall, cancel_event: Optional[asyncio.Event] = None
    ) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                success=False,
                result=f"Error: Invalid JSON arguments for tool '{name}': {e}",
            )
        except Exception as e:
            log.error("Tool '%s' failed: %s", name, e, exc_info=is_debug())
            return ToolResult(
                success=False, result=format_tool_error(name, e, tool_input)
            )
