...
class DiscoverProjectParams(BaseModel):
    use_cache: bool = Field(default=True, description="Whether to use cached project info")
...
class DiscoverProjectTool(BaseTool):
    ...
    async def execute(self, params: DiscoverProjectParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
        from rta_cli.discovery import discover_project
        try:
            result = await asyncio.to_thread(discover_project, self.working_directory, params.use_cache)
...
class GetFileContentsTool(BaseTool):
    ...
    async def execute(self, params: GetFileContentsParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
        from rta_cli.functions.get_file_content import get_file_contents
        try:
            result = await asyncio.to_thread(get_file_contents, self.working_directory, params.file_path)
...
class GetFilesInfoParams(BaseModel):
    directory: Optional[str] = Field(default=None, description="Directory to list, relative to working directory (default: root)")
...
class GetFilesInfoTool(BaseTool):
    ...
    async def execute(self, params: GetFilesInfoParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
        from rta_cli.functions.get_files_info import get_files_info
        try:
            result = await asyncio.to_thread(get_files_info, self.working_directory, params.directory)
            if result.startswith("Error"):
                return ToolResult(success=False, result=result)
            return ToolResult(success=True, result=result)
        except Exception as e:
...
class WriteFileTool(BaseTool):
    ...
    async def execute(self, params: WriteFileParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
        from rta_cli.functions.write_file import write_file
        try:
            result = await asyncio.to_thread(write_file, self.working_directory, params.file_path, params.content, params.overwrite)
...
class DeleteFileParams(BaseModel):
    file_path: str = Field(description="Path to the file to delete, relative to working directory")
...
class DeleteFileTool(BaseTool):
    ...
    async def execute(self, params: DeleteFileParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
        from rta_cli.functions.delete_file import delete_file
        try:
            result = await asyncio.to_thread(delete_file, self.working_directory, params.file_path)
            if result.startswith("Error") or result.startswith("Cancelled") or result.startswith("Failed"):
                return ToolResult(success=False, result=result)
            return ToolResult(success=True, result=result)
        except Exception as e:
            return ToolResult(success=False, result=f"Error: {e}")
...
class CreateDirParams(BaseModel):
    dir_path: str = Field(description="Path to the directory to create, relative to working directory")
...
class CreateDirTool(BaseTool):
    ...
    async def execute(self, params: CreateDirParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
        from rta_cli.functions.create_dir import create_dir
        try:
            result = await asyncio.to_thread(create_dir, self.working_directory, params.dir_path)
            if result.startswith("Error") or result.startswith("Failed"):
                return ToolResult(success=False, result=result)
            return ToolResult(success=True, result=result)
        except Exception as e:
...
class ApplyDiffTool(BaseTool):
    name = "apply_diff"
    description = "Applies a unified diff to one or more files in the working directory."
    ...
    async def execute(self, params: ApplyDiffParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
        from rta_cli.functions.apply_diff import apply_diff
        try:
            result = await asyncio.to_thread(apply_diff, self.working_directory, params.diff_text)
            if result.startswith("Error") or result.startswith("Failed"):
                return ToolResult(success=False, result=result)
            return ToolResult(success=True, result=result)
        except Exception as e:
...
class EditFileAstParams(BaseModel):
    ...
    action: str = Field(description="Refactoring action: rename_function or rename_class")
...
class EditFileAstTool(BaseTool):
    ...
    async def execute(self, params: EditFileAstParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
        from rta_cli.functions.edit_file_ast import edit_file_ast
        try:
            result = await asyncio.to_thread(edit_file_ast, self.working_directory, params.file_path, params.action, old_name=params.old_name, new_name=params.new_name)
            if result.startswith("Error") or result.startswith("Failed"):
                return ToolResult(success=False, result=result)
            return ToolResult(success=True, result=result)
        except Exception as e:
...
class ListSkillsTool(BaseTool):
    ...
    async def execute(self, params: ListSkillsParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
...
class SemanticSearchParams(BaseModel):
    query: str = Field(description="Natural language query (e.g., 'how is authentication handled?')")
...
class SemanticSearchTool(BaseTool):
    ...
    async def execute(self, params: SemanticSearchParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
        from rta_cli.functions.semantic_search import semantic_search
        try:
            result = await asyncio.to_thread(semantic_search, self.working_directory, params.query, params.limit)
            if result.startswith("Error"):
                return ToolResult(success=False, result=result)
            return ToolResult(success=True, result=result)
        except Exception as e:
...
class GetRepoSkeletonTool(BaseTool):
    ...
    async def execute(self, params: GetRepoSkeletonParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
...
class QuestionParams(BaseModel):
    header: Optional[str] = Field(default=None, description="Short label for the question (max 30 chars)")
...
class QuestionTool(BaseTool):
    ...
    async def execute(self, params: QuestionParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
        from rta_cli.questions import ask_question
        from rta_cli.utils import get_working_directory
        try:
            wd = get_working_directory()
            result = await asyncio.to_thread(ask_question, wd, header=params.header, question=params.question, options=params.options, multiple=params.multiple)
            return ToolResult(success=True, result=result)
        except Exception as e:
            return ToolResult(success=False, result=f"Error: {e}")
...
class GitStatusTool(BaseTool):
    ...
    async def execute(self, params: GitStatusParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
...
class GitDiffTool(BaseTool):
    ...
    async def execute(self, params: GitDiffParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
        from rta_cli.git import git_diff
        try:
            result = await asyncio.to_thread(git_diff, self.working_directory, params.staged)
            return ToolResult(success=True, result=result)
        except Exception as e:
            return ToolResult(success=False, result=f"Error: {e}")
...
class GitLogTool(BaseTool):
    ...
    async def execute(self, params: GitLogParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
...
class GitCommitParams(BaseModel):
    ...
    auto_add: bool = Field(default=True, description="Auto-stage modified files before committing")
...
class GitCommitTool(BaseTool):
    ...
    async def execute(self, params: GitCommitParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
        from rta_cli.git import git_commit
        try:
            result = await asyncio.to_thread(git_commit, self.working_directory, params.message, params.auto_add, params.force)
...
class GitCreatePrTool(BaseTool):
    ...
    async def execute(self, params: GitCreatePrParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
        from rta_cli.git import git_create_pr
        try:
            result = await asyncio.to_thread(git_create_pr, self.working_directory, params.title, params.body, params.base)
            if result.startswith("Error"):
                return ToolResult(success=False, result=result)
            return ToolResult(success=True, result=result)
        except Exception as e:
...
class GitBranchParams(BaseModel):
    ...
    create: Optional[str] = Field(default=None, description="Name of new branch to create")
...
class GitBranchTool(BaseTool):
    ...
    async def execute(self, params: GitBranchParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
        from rta_cli.git import git_branch
        try:
            result = await asyncio.to_thread(git_branch, self.working_directory, params.list, params.create, params.delete)
            if result.startswith("Error"):
                return ToolResult(success=False, result=result)
            return ToolResult(success=True, result=result)
        except Exception as e:
...
class WebSearchTool(BaseTool):
    ...
    async def execute(self, params: WebSearchParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
        from rta_cli.mcp.search import web_search
        try:
            result = await asyncio.to_thread(web_search, params.query, params.max_results)
            return ToolResult(success=True, result=result)
        except Exception as e:
            return ToolResult(success=False, result=f"Error: {e}")
...
class SequentialThinkingParams(BaseModel):
    ...
    is_revision: bool = Field(default=False, description="Whether this revises a previous thought")
    revises_thought: Optional[int] = Field(default=None, description="Which thought number to revise")
    branch_from_thought: Optional[int] = Field(default=None, description="Branch a new line of reasoning from this thought")
    branch_id: Optional[str] = Field(default=None, description="Unique ID for this reasoning branch")
    needs_more_thoughts: bool = Field(default=True, description="Whether more thoughts are still needed")
...
class SequentialThinkingTool(BaseTool):
    ...
    async def execute(self, params: SequentialThinkingParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
        from rta_cli.mcp.sequential_thinking import sequential_thinking
        try:
            result = await asyncio.to_thread(sequential_thinking, params.thought, params.thought_number, params.total_thoughts, params.next_thought_needed, params.is_revision, params.revises_thought, params.branch_from_thought, params.branch_id, params.needs_more_thoughts)
            return ToolResult(success=True, result=result)
        except Exception as e:
            return ToolResult(success=False, result=f"Error: {e}")
...
class MemorizeTool(BaseTool):
    ...
    async def execute(self, params: MemorizeParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
        from rta_cli.mcp.memory import memorize
        try:
            result = await asyncio.to_thread(memorize, params.key, params.value, params.tags)
            return ToolResult(success=True, result=result)
        except Exception as e:
            return ToolResult(success=False, result=f"Error: {e}")
...
class RecallTool(BaseTool):
    ...
    async def execute(self, params: RecallParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
...
class ForgetTool(BaseTool):
    ...
    async def execute(self, params: ForgetParams, cancel_event: Optional[asyncio.Event] = None) -> ToolResult:
