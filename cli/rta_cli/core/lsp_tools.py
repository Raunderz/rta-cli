...
class GetDiagnosticsParams(BaseModel):
    file_path: str = Field(
        description="Path to the file to check for errors and warnings"
    )
...
class GetDiagnosticsTool(BaseTool):
    ...
    async def execute(
        self, params: GetDiagnosticsParams, cancel_event: Optional[asyncio.Event] = None
    ) -> ToolResult:
        from rta_cli.functions.lsp_tools import get_diagnostics as sync_get_diagnostics

        try:
            result = await asyncio.to_thread(
                sync_get_diagnostics, self.working_directory, params.file_path
            )
...
class GoToDefinitionTool(BaseTool):
    ...
    description = (
        "Finds the location where a symbol (function, class, variable) is defined."
    )
    ...
    async def execute(
        self, params: GoToDefinitionParams, cancel_event: Optional[asyncio.Event] = None
    ) -> ToolResult:
        from rta_cli.functions.lsp_tools import (
            go_to_definition as sync_go_to_definition,
        )

        try:
            result = await asyncio.to_thread(
                sync_go_to_definition,
