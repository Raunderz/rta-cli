...
_TOOLS: dict[ToolName, _ToolConfig] = {
    ...
    "rg": _RgConfig(
        name="ripgrep", repo="BurntSushi/ripgrep", binary_name="rg", tag_prefix=""
    ),
}
...
async def _get_latest_version(client: httpx.AsyncClient, repo: str) -> str:
    resp = await client.get(
        f"https://api.github.com/repos/{repo}/releases/latest",
        headers={"User-Agent": "rta"},
    )
...
async def _download_tool(tool: ToolName) -> str:
    ...
    async with httpx.AsyncClient(timeout=120) as client:
        version = await _get_latest_version(client, config.repo)

        asset_name = config.get_asset_name(version, plat, arch)
        if not asset_name:
            raise RuntimeError(
                f"No binary available for {config.name} on {plat}/{arch}"
            )
        ...
        try:
            await _download_file(client, download_url, archive_path)
