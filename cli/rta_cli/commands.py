...
def chat(
    ...
    if list_sessions:
        ...
        for s in sessions[:15]:
            ...
            date = s["display_date"]
            console.print(
                f"ID: [cyan]{short_id}[/] | Project: [green]{project:<15}[/] | Date: {date}"
            )
        console.print(
...
def ask(
    ...
    chat_obj = RtaChat(
        workspace=workspace, session_id=resume, timeout=timeout, force=force
    )
    chat_obj._load_history()

    # Add system prompt if needed
    if not chat_obj.messages or not any(
        m.get("role") == "system" for m in chat_obj.messages
    ):
...
def skill(args):
    ...
    if not args.subcommand or args.subcommand == "list":
        ...
        if not skills:
            console.print(
                "[yellow]No skills installed. Add folders to ~/.rta/skills/[/yellow]"
            )
            return
        ...
        if not args.name:
            console.print(
                "[red]Error: Skill name required. (rta skill info <name>)[/red]"
            )
            return
        ...
        if not args.name:
            console.print(
                "[red]Error: Skill name required. (rta skill load <name>)[/red]"
            )
            return
        content = load_skill_content(args.name)
        if content:
    ...
    def error(self, message):
        ...
        if match:
            ...
            if bad_choice in typo_map:
                ...
                console.print(
                    f"[yellow]Hint: Assuming '{corrected}' instead of '{bad_choice}'[/yellow]"
                )
...
def main():
    ...
    # Global options
    parser.add_argument(
        "--clear-context", action="store_true", help="Clear chat history"
    )
    parser.add_argument("--workspace", help="Working directory")
    parser.add_argument("--no-cache", action="store_true", help="Ignore context")
    parser.add_argument(
        "--timeout", type=int, default=120, help="Timeout for shell commands in seconds"
    )
    parser.add_argument(
        "--force", action="store_true", help="Skip destructive action confirmations"
    )
    parser.add_argument("--resume", help="Resume a previous session by ID")
    parser.add_argument(
        "--list-sessions", action="store_true", help="List previous chat sessions"
    )
    ...
    parser.add_argument(
        ...
        nargs="?",
        const="deepseek-r1",
        help="Use local Ollama model (default: deepseek-r1)",
    )
    parser.add_argument(
        "--debug", action="store_true", help="Enable debug logging to ~/.rta/debug.log"
    )
    ...
    p_ask = subparsers.add_parser(
        "ask", help="Run a one-off agentic request (headless)"
    )
    ...
    p_skill.add_argument(
        "subcommand", nargs="?", choices=["list", "info", "load"], default="list"
    )
    p_skill.add_argument(
        "name", nargs="?", help="Skill name for 'info' or 'load' command"
    )
            ...
            for arg in unknown:
                if arg.startswith("-"):
                    ...
                    if raw_name in typo_map:
                        ...
                        console.print(
                            f"[yellow]Hint: Executing 'rta {cmd_name}' (matched from '{arg}')[/yellow]"
                        )
                        if cmd_name == "whoami":
                            return whoami()
                        if cmd_name == "status":
                        ...
                        if cmd_name == "logout":
                            return logout()
                        if cmd_name == "init":
                            return init(None)  # Needs args handling usually
