...
def load_context(
    workspace_dir: str = None, session_id: str = None, max_turns: int = 10
...
def save_context(workspace_dir: str, session_id: str, messages: list[dict]) -> None:
    ...
    if len(data) > 50:
        ...
        sorted_sessions = sorted(
            data.items(), key=lambda x: x[1].get("last_updated", "")
        )
...
def clear_context(workspace_dir: str = None, session_id: str = None) -> None:
    ...
    if session_id:
        ...
        abs_dir = os.path.abspath(workspace_dir)
        to_delete = [
            sid for sid, sdata in data.items() if sdata.get("workspace_dir") == abs_dir
        ]
