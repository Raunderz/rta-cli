...
class RtaChat:
    ...
    def _load_history(self):
        ...
        msgs, sid, sw = load_context(
            workspace_dir=self.workspace, session_id=self.session_id, max_turns=10
        )
        self.messages = msgs
        if sid:
            self.session_id = sid
    ...
    def print_header(self):
        ...
        user_display = "authenticated" if self.privacy else self.user
        console.print(f"\nRta Cli {self.version} | {user_display} | {self.workspace}\n")
    ...
    def handle_slash_command(self, command_str):
        ...
        if cmd_name == "help":
            ...
            console.print(
                "  /review        - Toggle review mode (read-only tools only)"
            )
            ...
            console.print(
                "[dim]You can also run any Rta CLI command here (e.g., /skill info, /status)[/dim]\n"
            )
            return
            ...
            if self.review_mode:
                for msg in self.messages:
                    if msg.get("role") == "system":
                        ...
                        break
                console.print(
                    "[bold yellow]Review mode ON — mutating tools are blocked.[/bold yellow]"
                )
            else:
                for msg in self.messages:
                    if msg.get("role") == "system":
                        ...
                        break
                console.print(
                    "[bold green]Review mode OFF — all tools available.[/bold green]"
                )
            return
        ...
        if cmd_name == "load_history":
            self._load_history()
            console.print(
                f"[bold green]Loaded {len(self.messages)} messages from history.[/bold green]"
            )
            return
        ...
        try:
            ...
            if cmd_name == "skill" and args and args[0] == "load" and result:
                ...
                if has_hidden_content(result):
                    ...
                    choice = (
                        console.input("[bold]Load anyway? (y/N): [/bold]")
                        .strip()
                        .lower()
                    )
                    if choice != "y":
                ...
                self.messages.insert(
                    ...
                    {
                        "role": "system",
                        "content": f'<activated_skill name="{skill_name}">\n{result}\n</activated_skill>',
                    },
                )
                console.print(
                    f"[bold green]Skill '{skill_name}' loaded successfully.[/bold green]"
                )

                # Auto-trigger an activation turn to ensure context is updated
                self._trigger_activation_turn(
                    f"Skill '{skill_name}' activated. Please acknowledge."
                )
        ...
        while True:
            try:
                ...
                if think_mode:
                    ...
                    console.print(
                        f" [dim]Prompt:[/dim] {user_input.replace('think_it', hl)}"
                    )
                ...
                status = console.status(
                    random.choice(LOADING_MESSAGES), spinner="pacman"
                )
                status.start()
