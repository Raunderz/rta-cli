...
from rta_cli.functions.get_file_content import (
    get_file_contents,
...
from rta_cli.functions.get_repo_skeleton import (
    get_repo_skeleton,
    schema_get_repo_skeleton,
)
from rta_cli.functions.lsp_tools import (
    get_diagnostics,
    go_to_definition,
...
from rta_cli.mcp.sequential_thinking import (
    sequential_thinking,
    schema_sequential_thinking,
)
from rta_cli.mcp.memory import (
    memorize,
    recall,
    forget,
    schema_memorize,
...
from rta_cli.mcp import (
    call_mcp_tool,
    list_mcp_tools,
    map_mcp_to_openai_schema,
    load_mcp_config,
)
from rta_cli.discovery import (
    discover_project,
    get_lint_command,
    schema_discovery,
)
...
def explain_error(tool: str, error: str, args: dict) -> str:
    ...
    if tool == "run_command":
        if "not found" in error or "no such file" in error.lower():
            cmd = (
                args.get("command", "").split()[0] if args.get("command") else "command"
            )
            return (
                f"{error} - Suggestion: Check if '{cmd}' is installed or use full path."
            )
        ...
        if not allowed:
            return {
                "functionResponse": {
                    ...
                    "response": {
                        "name": name,
                        "content": f"Blocked: '{name}' is not available in review mode. Only read-only tools are allowed.",
                    },
                }
            }
...
def _call_backend_stream(
    ...
    if provider_obj:
        import asyncio
        from rta_cli.core.types import (
            UserMessage,
            AssistantMessage,
            SystemMessage,
            ToolMessage,
        ...
        for m in messages:
            ...
            if role == "user":
                obj_messages.append(UserMessage(content=content))
            elif role == "assistant":
                obj_messages.append(
                    AssistantMessage(content=content, tool_calls=m.get("tool_calls"))
                )
            elif role == "system":
                obj_messages.append(SystemMessage(content=content))
            elif role == "tool":
                obj_messages.append(
            ...
            async def run_provider():
                ...
                async for event in provider_obj.stream(obj_messages, tools=tools):
                    if isinstance(event, TextDeltaEvent):
                        ...
                        current_tool_calls[event.tool_call_id]["function"][
                            "arguments"
                        ] += event.delta
                        ...
                        yield {
                            "type": "error",
                            "content": f"{event.message}: {event.details}",
                        }

                if current_tool_calls:
                    yield {
                        "type": "tool_calls",
                        "content": list(current_tool_calls.values()),
                    }
                yield {"type": "done"}
    ...
    try:
        ...
        yield {
            "type": "error",
            "content": "Cannot reach Rta server. Check your connection.",
        }
    except httpx.TimeoutException:
...
def stream_agent(
    ...
    for i in range(max_iterations):
        ...
        tools = [
            ...
            if not read_only or t["function"]["name"] in READ_ONLY_TOOLS
        ]
        ...
        if not text_buffer and not tool_calls and not stream_usage:
            yield {
                "type": "error",
                "content": "Empty response from server (streaming not supported?).",
            }
            return
        ...
        usage["total_tokens"] += stream_usage.get(
            "prompt_tokens", 0
        ...
        def execute_one(tc, force_flag=False, read_only_flag=False):
            ...
            for attempt in range(max_retries + 1):
                ...
                if "(PERMISSION_REQUIRED)" in content and not force_flag:
                    path_match = (
                        content.split("Error : ")[1].split(" is ignored")[0]
                        if "Error : " in content
                        else "this path"
                    )
                    console.print(
                    ...
                    choice = (
                        console.input("[bold]Allow access? (y/N): [/bold]")
                        .strip()
                        .lower()
                    )
                    if choice == "y":
                ...
                if "(HIDDEN_CONTENT_PERMISSION_REQUIRED)" in content and not force_flag:
                    skill_name = (
                        content.split("Skill '")[1].split("' contains")[0]
                        if "Skill '" in content
                        else "this skill"
                    )
                    console.print(
                    ...
                    console.print(
                        "[dim]Hidden content can be used for malicious prompt injection.[/dim]"
                    )
                    choice = (
                        console.input("[bold]Activate anyway? (y/N): [/bold]")
                        .strip()
                        .lower()
                    )
                    if choice == "y":
        ...
        for mode, group_tcs in groups:
            if mode == "parallel" and len(group_tcs) > 1:
                ...
                with ThreadPoolExecutor(
                    max_workers=min(len(group_tcs), 10)
                ) as executor:
                    results = list(
                        executor.map(
                            lambda tc: execute_one(tc, force, read_only), group_tcs
                        )
                    )
...
def run_agent(
    ...
    for event in stream_agent(
        ...
        if event["type"] == "text":
            ...
            if args and args != "{}":
                final_text += (
                    f"*(Executed tool: `{event['content']}` with args: `{args}`)*\n\n"
                )
            else:
