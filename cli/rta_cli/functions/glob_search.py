    ...
    try:
        ...
        try:
            ...
            matches = [
                m
                for m in matches
                if os.path.commonpath([m, abs_working_dir]) == abs_working_dir
            ]
        ...
        matches = [
            m
            for m in matches
            if not gf.is_ignored(
                os.path.join(abs_working_dir, m), allow_ignored=allow_ignored
            )
        ]

        # Convert to relative paths for cleaner output
        relative_matches = [
            m if not os.path.isabs(m) else os.path.relpath(m, abs_working_dir)
            for m in matches
        ]
