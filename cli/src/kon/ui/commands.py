...
class CommandsMixin:
    ...
    async def _rta_whoami_flow(self) -> None:
        ...
        if not api_key:
            chat.add_info_message("Not logged in to RTA. Use /login rta <key>", error=True)
            return

        headers = {
            "X-API-KEY": api_key,
            "X-Device-ID": kon_auth.get_device_id(),
        ...
        try:
            ...
            if resp.status_code == 200:
                data = resp.json()
                chat.add_info_message(
                    f"[bold]RTA Status:[/bold] Authenticated\n"
                    f"[bold]Tier:[/bold]       {data.get('tier', '?')}\n"
                    f"[bold]User ID:[/bold]    {data.get('user_id', '?')}"
                )
            else:
                chat.add_info_message(f"RTA Auth error ({resp.status_code})", error=True)
        except Exception as e:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                chat.add_info_message(
                    ...
                    f"[bold]Tier:[/bold]         {d.get('tier', '?')}\n"
                    f"[bold]Calls today:[/bold]  {d.get('calls_today', '?')} / {d.get('calls_limit', '?')}\n"
                    f"[bold]Tokens today:[/bold] {d.get('tokens_today', '?')} / {d.get('tokens_limit_day', '?')}\n"
                    f"[bold]Tokens/mo:[/bold]    {d.get('tokens_used_month', '?')} / {d.get('tokens_limit_month', '?')}"
                )
            else:
                chat.add_info_message(f"RTA Server error ({resp.status_code})", error=True)
        except Exception as e:
            chat.add_info_message(f"RTA Connection error: {e}", error=True)
    ...
    def _handle_init_command(self, args: str) -> None:
        ...
        if os.path.exists(project_name):
            chat.add_info_message(f"Error: '{project_name}' already exists", error=True)
            return
