...
async def call_openrouter(messages, model, tools, api_key, max_tokens) -> dict:
    ...
    try:
        ...
        if response.status_code == 429:
            ...
            raise ProviderDownError(f"OpenRouter server error: {response.status_code}")
        ...
        raise ProviderDownError(f"OpenRouter HTTP error: {e}")
...
async def call_openrouter_stream(messages, model, tools, api_key, max_tokens):
    url = "https://openrouter.ai/api/v1/chat/completions"
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Content-Type": "application/json",
