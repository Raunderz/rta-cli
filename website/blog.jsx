...
When you send \`"model": "rta-auto"\`, Rta routes through its provider chain — Groq, Cerebras, SambaNova, OpenRouter, Gemini — with automatic fallback. If one provider is down, the next one picks up. You get the speed of Groq with the reliability of a five-provider fallback chain.

No API keys to manage on your end. One key, all providers.
...
Free tier includes 10 calls per day. Paid tiers are available on our [Pricing page](/pricing).

## What's Next

This is the foundation. We'll be expanding the API with:

- **More model options** — request specific models instead of auto-routing
- **Tool calling** — pass function definitions and get structured tool call responses
- **Webhooks** — push-based integrations for long-running tasks

Full documentation is available at [/api](/api).
